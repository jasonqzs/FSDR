...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet32',
                    ...
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet32)')
parser.add_argument('--resize', default="224", type=int, help='resize for data')
parser.add_argument('--loss_type', default="CE", type=str, help='loss type')
parser.add_argument('--imb_type', default="exp", type=str, help='imbalance type')
parser.add_argument('--imb_factor', default=5, type=float, help='imbalance factor')
parser.add_argument('--train_rule', default='224', type=str, help='data sampling strategy for train loader')
...
def main():
    ...
    args.store_name = '_'.join(
        [str(args.workers), str(args.epochs), args.dataset, args.arch,
         str(args.imb_factor), str(args.resize), "5"])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    dataset_root = "/data/ch/tmp/pycharm_project_191/fsdr/malimg/malimg_dataset/bal_dataset_5"

    # 创建ImageFolder对象
    train_dataset = ImageFolder(root=dataset_root + "/train", transform=transform_train)
