...
parser = argparse.ArgumentParser(description='PyTorch Malevis Training')
parser.add_argument('--dataset', default='Malevis', help='dataset setting')
...
parser.add_argument('--imb_factor', default=20, type=float, help='imbalance factor')
...
def main():
    ...
    args.store_name = '_'.join(
        [str(args.workers), str(args.epochs), args.dataset, args.arch, args.loss_type, args.imb_type,
         str(args.imb_factor), "class:25"])
...
def main_worker(gpu, ngpus_per_node, args):
    global best_acc1, best_acc_1, best_macro_acc
    args.gpu = gpu
    ...
    dataset_root = current_directory + "/malevis_dataset"

    # 创建ImageFolder对象
    train_dataset = ImageFolder(root=dataset_root + "/train_20", transform=transform_train)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, optimizer, epoch, args, log_training, tf_writer, ratio, log_epoch, train_propertype,
              weights_old)

        if epoch >= 30: #150
            # evaluate on validation set
            acc1, acc_1, macro_acc = validate(val_loader, model, epoch, args, log_testing, tf_writer)
            # remember best acc@1 and save checkpoint
            is_best = acc1 > best_acc1
            best_acc1 = max(acc1, best_acc1)

            best_acc_1 = max(acc_1, best_acc_1)
            best_macro_acc = max(macro_acc, best_macro_acc)

            # tf_writer.add_scalar('acc/test_top1_best', best_acc1, epoch)
            output_best = 'Best Prec@1: %.3f\n' % (best_acc1)
            print(output_best)
            print(best_acc_1 , best_macro_acc)
...
def validate(val_loader, model, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    with torch.no_grad():
        ...
        # 计算总体分类准确率
        overall_accuracy = np.sum(cls_hit) / np.sum(cls_cnt)
        # 计算宏平均准确率
        macro_avg_accuracy = np.mean(cls_acc)
        print(overall_accuracy, macro_avg_accuracy)
    ...
    return top1.avg, overall_accuracy, macro_avg_accuracy
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch < 5:
        ...
        # lr = args.lr * ((0.1 ** int(epoch >= 100)) * (0.01 ** int(epoch >= 120)))
